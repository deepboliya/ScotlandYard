/**
 * fast_policy_v2.1.cpp
 *
 * Dense-table Scotland Yard solver (generalized, up to 5 detectives).
 *
 * Differences vs fast_policy_v2.cpp:
 *   1) No unordered_map. Memo uses fixed-size arrays indexed by
 *      (depth_left, is_x_turn, x_pos, sorted_detective_tuple_with_repetition).
 *   2) Thread-safe memoization via atomics (no mutexes).
 *   3) Optional threading:
 *        - BFS distance precompute across source nodes
 *        - top-level Mr. X move evaluation
 *
 * Build:
 *   g++ -O3 -std=c++17 -pthread -o fast_policy_v2_1 fast_policy_v2.1.cpp
 *
 * Usage:
 *   ./fast_policy_v2_1 --map maps/full_map.txt --mrx 100 --detectives 1 199 --max-rounds 12 --threads 8 --suffix exp1
 */
...
// Dense memo arrays.
// memo_state: 0 = not computed, 1 = being computed, 2 = computed
static unique_ptr<atomic<uint8_t>[]> memo_state;
static vector<uint8_t> memo_value;
static vector<uint16_t> memo_depth_used;
...
static int solve(int depth_left, int x_pos, int* dets, int nd,
    ...
    while (true) {
        uint8_t st = memo_state[idx].load(memory_order_acquire);
        if (st == 2) {
            int val = (int)memo_value[idx];
            int depth_used = (int)memo_depth_used[idx];

            // Same reuse rule as fast_policy_v2:
            ...
            uint8_t expected_done = 2;
            if (memo_state[idx].compare_exchange_weak(expected_done, 1,
        ...
        if (st == 1) {
            ...
            // With round-invariant keys (depth not in key), waiting here can
            // deadlock across threads (A waits on B while B waits on A).
            // Fall back to local computation without claiming/writing the slot.
            break;
        }
        uint8_t expected = 0;
        if (memo_state[idx].compare_exchange_weak(expected, 1, memory_order_acq_rel, memory_order_acquire)) {
            owns_slot = true;
    ...
    if (owns_slot) {
        memo_value[idx] = (uint8_t)result;
        memo_depth_used[idx] = (uint16_t)depth_left;
        memo_state[idx].store(2, memory_order_release);
    }
    return result;
}
...
static void build_json_maps() {
    ...
    enumerate_det_tuples_rec(0, 1, dets, [&](const int* tuple) {
        for (int x = 1; x <= num_nodes; ++x) {
            ...
            if (memo_state[idx_x].load(memory_order_acquire) == 2 && mrx_policy_set[idx_x]) {
                string key = state_to_key(true, x, tuple, ND);
                sorted_survival[key] = (int)memo_value[idx_x];
                sorted_mrx[key] = (int)mrx_best_move[idx_x];
            }

            size_t idx_d = state_index(x, tuple, ND, false);
            if (memo_state[idx_d].load(memory_order_acquire) == 2 && det_policy_set[idx_d]) {
                string key = state_to_key(false, x, tuple, ND);
                sorted_survival[key] = (int)memo_value[idx_d];
                vector<int> mv;
                mv.reserve(ND);
    ...
    enumerate_det_tuples_rec(0, 1, dets, [&](const int* tuple) {
        for (int x = 1; x <= num_nodes; ++x) {
            ...
            if (memo_state[idx_d].load(memory_order_acquire) == 2 && det_policy_set[idx_d]) {
                ...
                r.data[1 + 2 * nd] = memo_value[idx_d];
...
int main(int argc, char *argv[]) {
    ...
    memo_state = make_unique<atomic<uint8_t>[]>(total_states);
    memo_value.assign(total_states, 0);
    memo_depth_used.assign(total_states, 0);
    ...
    for (size_t i = 0; i < total_states; ++i)
        memo_state[i].store(0, memory_order_relaxed);

    cout << "Dense memo states: " << total_states << "\n";
    cout << "Approx memo memory: "
            << (double)(total_states * (sizeof(uint8_t) + sizeof(uint8_t) + sizeof(uint16_t))) / (1024.0 * 1024.0)
         << " MiB\n";
