...
// String key for JSON output  (includes round for back-compat).
...
// ── Reconstruct round-annotated states (for JSON back-compat) ──────────
//
// Walk the game tree from the initial state, annotating each visited
// position with its round number.  This produces the same JSON keys as
// fast_policy.cpp even though our internal memo is round-free.

static map<string, int>         sorted_mrx;
static map<string, vector<int>> sorted_det;
static map<string, int>         sorted_depths;

static void walk_full(int round, int x_pos, int* dets, int nd, bool is_x_turn) {
    string skey = state_to_key(round, is_x_turn, x_pos, dets, nd);
    if (sorted_depths.count(skey)) return;

    // Terminal: caught.
    for (int i = 0; i < nd; ++i)
        if (x_pos == dets[i]) {
            sorted_depths[skey] = round;
            return;
        }

    // Terminal: survived all rounds.
    if (round >= MAX_ROUNDS && is_x_turn) {
        sorted_depths[skey] = MAX_ROUNDS;
        return;
    }

    uint64_t key = encode_state(x_pos, dets, nd, is_x_turn);
    auto memo_it = memo.find(key);
    if (memo_it == memo.end()) return;

    // Convert relative survival → absolute round for JSON compat.
    int relative = min(memo_it->second.first, MAX_ROUNDS - round);
    int absolute = round + relative;
    if (absolute > MAX_ROUNDS) absolute = MAX_ROUNDS;
    sorted_depths[skey] = absolute;

    if (is_x_turn) {
        auto pit = mrx_policy.find(key);
        if (pit != mrx_policy.end())
            sorted_mrx[skey] = pit->second;

        // Distance prune — if pruned, only follow optimal move.
        if (nd > 0) {
            int min_dist = 9999;
            for (int i = 0; i < nd; ++i)
                min_dist = min(min_dist, dist_matrix[x_pos][dets[i]]);
            if (min_dist > MAX_ROUNDS - round) {
                if (pit != mrx_policy.end())
                    walk_full(round + 1, pit->second, dets, nd, nd == 0);
                return;
            }
        }
        ...
        const auto &moves = adj[x_pos];
        for (int m : moves)
            walk_full(round + 1, m, dets, nd, nd > 0 ? false : true);

    } else {
        auto dit = det_policy.find(key);
        if (dit != det_policy.end()) {
            sorted_det[skey] = dit->second;
            int new_dets[5];
            for (int i = 0; i < nd; ++i) new_dets[i] = dit->second[i];
            sort(new_dets, new_dets + nd);
            walk_full(round, x_pos, new_dets, nd, true);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    // ── reconstruct round-annotated states for JSON ────────────
    cout << "Reconstructing round-annotated states for JSON..." << flush;
    walk_full(0, mrx_start, dets_arr, nd, true);
    cout << " done. (" << sorted_depths.size() << " annotated states)\n";
